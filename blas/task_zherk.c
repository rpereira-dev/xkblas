...
#define TASK_NAME zherk
#define STRNAME  "zherk"
#define NAME2(x,s) x##_##s
#define NAME(x) NAME2(x,zherk)
#define PNAME(x) zherk##_##x
#define NPARAM 2
...
#define TASK_FLOPS FLOPS_ZHERK(arg->n,arg->k)
#define TASK_DATA  DATA_ZHERK(arg->n,arg->k)
...
void INSERT_TASK_zherk(
    int uplo, int trans,
    size_t n, size_t k,
    CFloat64_t alpha, xkblas_matrix_descr_t *Ah, size_t Am, size_t An, size_t lda,
    CFloat64_t beta,  xkblas_matrix_descr_t *Bh, size_t Bm, size_t Bn, size_t ldb)
{
    kaapi_task_t* task;
...
{
  NAME(Arg)* arg = (NAME(Arg)*)kaapi_task_getargs(task);
  cublasZherk((cublasHandle_t)handle,
